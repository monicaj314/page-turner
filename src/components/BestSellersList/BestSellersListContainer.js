...
class BestSellersListContainer extends React.Component {
  componentDidMount() {
    ...
    console.log(
      `Best Sellers just mounted.  Fetching books for:${selectedCategoryId}`
    );
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.selectedCategoryId !== nextProps.selectedCategoryId) {
      console.log(
        `Received update for SELECTED CATEGORY prop: ${nextProps.selectedCategoryId} - fetching best sellers`
      );
  ...
  render() {
    return <BestSellersList {...this.props} />;
  }
}
