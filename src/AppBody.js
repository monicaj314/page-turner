...
class AppBody extends React.Component {
  constructor(props) {
    ...
    if (this.props.match.params.id) {
      ...
      console.log(
        `App initialized.  Found a category in the URL: ${categoryUrlParam}`
      );
    } else {
      console.log(
        `App initialized.  Did not find a category in the URL.  Setting to default ${categoryUrlParam}`
      );
    }
    this.state = {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (currentUrlCategoryParam !== nextUrlCategoryParam) {
      console.log(
        "Found different category in URL. Dispatching update for " +
  ...
  componentDidMount() {
    console.log(
      `Init loading categories and updating selected category to: ${this.state
        .categoryUrlParam}`
    );
